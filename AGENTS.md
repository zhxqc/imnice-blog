...
- 完成一个独立模块后，先运行相关检查并报告结果；默认不自动提交或推送，只有用户明确要求时才执行 Git 操作。
- 开始新模块前先检查 `git status`。如果发现上一个模块留下的已暂存改动，先核对并收口它，再开始新的文件修改。
- 用户要求提交时，提交前检查 `git diff` 和 `git status`，只暂存当前模块相关文件；提交后检查工作区，确认没有遗漏或误纳入其他改动。
- 用户同时要求推送时，提交成功后再推送当前分支到已配置的远程仓库：`git push origin <当前分支>`。
