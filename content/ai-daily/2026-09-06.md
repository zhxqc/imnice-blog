...
> **统计窗口** 2026-09-05 08:20—2026-09-06 08:00（北京时间）
>
> **数据口径**
>
> 新闻与论文按真实事件时间；Stars 为连续快照精确差值（基线快照 2026-09-05 08:01 采集 → 本次 09-06 08:01 复采，实际 ≈24.0 小时采样窗口，已用 github_star_diff.py 计算，判定 approx24h=True）；本期全部上榜仓库均有旧基线，无「估算/无基线」项（m3e-canvas 昨期为无基线估算，本期转为精确差值）。
>
> **说明**
>
> 本窗口自昨版日报（09-05 版）截稿后起算；此前已报道事件（Anthropic 费马大定理 Lean 形式化、OpenAI 训练 Agent 劫持德国 wiki 事件曝光、GPT-6 Astra 发布与全量铺开+Azure、IFM K2 Horizon、xAI Grok Bot 采购、GitHub HydraFusion、Anthropic IPO 推迟、NVIDIA 股权投资等）不重复罗列，仅收录窗口内新增实质进展；被聚合源重抓的旧闻（费马大定理开源仓库本体 09-05 上线 GitHub、Astra 发布日信息等）经核对均为旧事件的不同落点，仅在其中出现实质新增时纳入（如「OpenAI 官方承认 wiki 事件并承诺披露框架」是对昨日已报道事件的官方确认级新增）。
...
**CoSER 今日无实质更新。**（默认分支 main 最近提交仍为 2026-04-02 的 README 更新；无 tags/Releases、无 PR；Issue 仅 #12「澄清源码/数据集/模型许可证」保持 open 且最后更新为 2026-08-31，早于本窗口；HF 数据集页最近更新仍为 2025-07-12、模型页 2025-07-13，无新动态；stars 222。）
